...
export default class StrengthLevelService {
    ...
    static async getStrengthLevel(
        ...
        const strengthLevelExercises = standardMen.exercises
            .map(x => x.exerciseName.toLocaleLowerCase().trim());
        const initialExercises = initialData.categories.reduce(
            (acc, currentValue) => acc.concat(
                currentValue.exercises.map(x => x.toLocaleLowerCase().trim())
                ),
            [] as string[]
        );
        console.log(initialExercises);
        const diff = initialExercises.filter(x => !strengthLevelExercises.includes(x));
        console.log(diff);
        console.log(strengthLevelExercises);

        const exerciseStrLevel = standardMen.exercises.find(x => 
           x.exerciseName.toLowerCase() === exerciseName0.toLowerCase()
        )
        const bodyRange = exerciseStrLevel.ranges.find(x => x.rangeStart >= bodyWeight)
        ...
        console.log(slided)
        const index0 = slided.findIndex(x => x[0] < oneRepMax && x[1] > oneRepMax)
        console.log(index0);
        const position = (oneRepMax - slided[index0][0]) / (slided[index0][1] - slided[index0][0]);
