...
export default function ExerciseSession(props: ExerciseSessionProps) {
  ...
  const { data: exerciseUnits } =
    ExerciseUnitQueries.listAllExerciseUnitsByDate(props.selectedDate);
  ...
  const findExerciseForSession = () => {
    if (categories && exerciseUnits) {
      ...
      safeArray(categories).forEach((category) =>
        exercisesDone.set(category.name, [])
      );
      safeArray(exerciseUnits).forEach((exercise) => {
        const category = categories.find((cat) =>
          cat.exercises.includes(exercise.exerciseName)
        );
        const exercises = exercisesDone
          .get(category.name)
          .concat([exercise.exerciseName]);
