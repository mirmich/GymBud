...
export default function SwipeList(props: SwipeListProps) {
  ...
  const rowTranslateAnimatedValues = Array(999)
    .fill("")
  ...
  const { data: exerciseUnit } =
    ExerciseUnitQueries.getExerciseUnitByNameAndDate(
      props.exerciseName,
      props.date
    );
    ...
    return (
      ...
      >
        <Pressable
          key={index}
          onPress={() =>
            selectedSetMutation.mutateAsync({
              index: index,
              unit: { weight: item.amount, reps: item.reps },
              operation: "modify",
            } as Selected)
          }
  ...
  const onSwipeValueChange = (swipeData) => {
    ...
    if (value < -Dimensions.get("window").width && !animationIsRunning) {
      ...
      Animated.timing(rowTranslateAnimatedValues[swipeData.key], {
        ...
        const removed: WeightAndReps[] = newData.splice(
          parseInt(swipeData.key),
          1
        );
        console.log(removed);
  ...
  const transformDataForPresentation = (exerciseUnit: ExerciseUnit) => {
    if (exerciseUnit) {
      return safeArray(exerciseUnit.weightAndReps as WeightAndReps[])
        .filter(Boolean)
